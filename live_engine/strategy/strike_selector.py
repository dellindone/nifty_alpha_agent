...
class NSEBSEStrategy:
    def select(self, processed, atm, direction, mode="NORMAL", instrument=None) -> dict | None:
        try:
            candidates = []
            ...
            for target in candidates_ordered:
                ...
                if spread_pct < Equity.SPREAD_THRESHOLD_PCT:
...
class StrikeSelector:
    def select(self, processed: list, atm: float, direction: str, mode: str = "NORMAL", instrument=None) -> dict | None:
        return NSEBSEStrategy().select(processed, atm, direction, mode, instrument=instrument)
