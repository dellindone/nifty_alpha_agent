...
class CandlePoll:
    ...
    def _run_candle_poll(self, now_ist: datetime) -> None:
        try:
            with self._e._tick_lock:
                ...
                for symbol in self._e.executor.cancel_expired_pending(now_ist.astimezone(timezone.utc)):
            ...
            if now_ist.hour > 15 or (now_ist.hour == 15 and now_ist.minute >= 15):
                ...
                trade_signal = self._e.signal_handler.process(prediction=prediction, feature_row=feature_row, instrument=self._e.instrument, daily_pnl=self._e._last_daily_pnl, daily_trade_count=self._e._last_daily_count, open_trade_count=len(self._e.executor.open_trades()))
            ...
            self._e._log_poll(now_ist, signal_label, len(self._e.executor.open_trades()))
            self._e._print_live_display(now_ist)
        except Exception as exc:
    ...
    def _fetch_live_frames(self, now_ist: datetime) -> dict[str, pd.DataFrame]:
        ...
        frame_D   = _get_cached_frame(sym, "D",  lambda s, r: _fetch(s, r, bars=300, days_back=365))
