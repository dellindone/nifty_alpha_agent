...
from features.pattern_context import compute_pattern_context
from features.regime import RegimeDetector
from features.session_features import compute_session_features
from features.vix_features import compute_vix_features
...
def _add_cross_tf_context(frame: pd.DataFrame) -> pd.DataFrame:
    contextual = frame.copy()
    zero_series = pd.Series(0, index=contextual.index, dtype=float)

    def _col(name: str) -> pd.Series:
        if name in contextual.columns:
            return pd.to_numeric(contextual[name], errors="coerce").fillna(0.0)
        return zero_series

    bull_score = (
        _col("ema_stack_bull_15m")
        + _col("ema_stack_bull_60m")
        + _col("ema_stack_bull_D")
        + _col("price_above_ema20_15m")
        + _col("price_above_ema20_60m")
        + _col("price_above_ema50_15m")
        + _col("price_above_ema50_60m")
        + _col("adx_strong_15m")
        + _col("adx_strong_60m")
    )
    bear_score = (
        _col("ema_stack_bear_15m")
        + _col("ema_stack_bear_60m")
        + _col("ema_stack_bear_D")
        + (1.0 - _col("price_above_ema20_15m"))
        + (1.0 - _col("price_above_ema20_60m"))
        + (1.0 - _col("price_above_ema50_15m"))
        + (1.0 - _col("price_above_ema50_60m"))
        + _col("adx_strong_15m")
        + _col("adx_strong_60m")
    )
    ...
    regime_bias = (
        _col("trend_regime_15m").map(regime_map).fillna(0.0)
        + _col("trend_regime_60m").map(regime_map).fillna(0.0)
        + _col("trend_regime_D").map(regime_map).fillna(0.0)
    )

    contextual["bull_context_score"] = bull_score
    contextual["bear_context_score"] = bear_score
    contextual["htf_regime_bias_score"] = regime_bias
    contextual["market_context_score"] = bull_score - bear_score + regime_bias
    contextual["long_context_ok"] = ((bull_score >= 5.0) & (regime_bias >= 0.0)).astype(int)
    contextual["short_context_ok"] = ((bear_score >= 5.0) & (regime_bias <= 0.0)).astype(int)
    return contextual
...
class FeatureEngineer:
    ...
    def _prepare_5m_frame(
        ...
        featured = compute_session_features(featured)
        featured = compute_institutional_context(featured)
    ...
    def _build_from_frames(
        ...
        assembled = _add_cross_tf_context(assembled)
