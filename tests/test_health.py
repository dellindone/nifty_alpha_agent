...
def test_monitor_transitions_and_recovery():
    h, r, m = AgentHealth(), FakeReporter(), None
    m = HealthMonitor(h, r)
    h.update("fyers_candle_api", "warn", "HTTP 503"); h.update("feature_pipeline", "warn", "NaN"); m.check_and_alert()
    h.update("fyers_candle_api", "warn", "HTTP 503"); h.update("feature_pipeline", "warn", "NaN"); m.check_and_alert()
    h.update("fyers_websocket", "critical", "no ticks"); h.update("feature_pipeline", "critical", "NaN"); m.check_and_alert()
    h.update("fyers_candle_api", "ok", ""); h.update("fyers_websocket", "ok", ""); h.update("feature_pipeline", "ok", ""); m.check_and_alert()
    assert "DEGRADED" in r.messages[0] and "CRITICAL" in r.messages[1] and "RECOVERED" in r.messages[2] and len(r.messages) == 3
